...
/*commonTabLayout.setOnClickListener(new View.OnClickListener() {
    ...
    public void onClick(View v) {
       ...
       // mPresenter.getKData(System.currentTimeMillis() - 2*24 * 60 * 60 * 1000L, "1", code, System.currentTimeMillis()- 24 * 60 * 60 * 1000L);
        mPresenter.getKData(System.currentTimeMillis()-12*60*60*1000L,"1",code,System.currentTimeMillis());
    ...
    private void tabSelect(int position) {
       ...
       // kChartView.setDrawMinuteStyle(false);
        ...
        if(position==1){
            mPresenter.getKData(System.currentTimeMillis()-5*24*60*60*1000L,"15",code,System.currentTimeMillis());
            currenttype= "15";
        ...
        if(position==4){
            mPresenter.getKData(System.currentTimeMillis()-300*24*60*60*1000L,"1d",code,System.currentTimeMillis());
            currenttype= "1d";
            ...
            if (kChartMorePop == null) {
              ...
              //  View tvOne = contentView.findViewById(R.id.tvOne);
                ...
                ClickUtil.click(tvThirty1, () -> {
                    ...
                    tabSelect(7);
                    mPresenter.getKData(System.currentTimeMillis()-5*24*60*60*1000L,"30",code,System.currentTimeMillis());
                    commonTabLayout.setCurrentTab(5);
                    currenttype= "30";
                    tvMore.setText("30M");
                    mPresenter.initSocket(code,"30min");
                ...
                ClickUtil.click(tvWeek, () -> {
                    ...
                    tabSelect(8);
                    mPresenter.getKData(System.currentTimeMillis()-300*7*24*60*60*1000L,"1w",code,System.currentTimeMillis());
                    commonTabLayout.setCurrentTab(5);
                    currenttype= "1w";
...
//            ImageUtil.setImage(R.mipmap.lib_loading,ivLoading);
            ...
            if (isLastNewData(data)) {
              ...
              /*  switch (currenttype) {
                    ...
                    case "1h":
                        ...
                        break;
                    case "4h":
                        mPresenter.getKData(System.currentTimeMillis() - 12* 24 * 60 * 60 * 1000L, "4h", code, System.currentTimeMillis());
                        break;
                    case "1d":
                        mPresenter.getKData(System.currentTimeMillis() - 60 * 24 * 60 * 60 * 1000L, "1d", code, System.currentTimeMillis());
                        break;
                    case "1w":
                        mPresenter.getKData(System.currentTimeMillis() - 60 * 7 * 24 * 60 * 60 * 1000L, "1w", code, System.currentTimeMillis());
                        break;
                    case "1m":
                        mPresenter.getKData(System.currentTimeMillis() - 300 * 7 * 24 * 60 * 60 * 1000L, "1m", code, System.currentTimeMillis());
                        break;
                ...
                // mAdapter.changeLastItemClosePrice(new BigDecimal(data.getClosePrice()).floatValue());
              /* Stock stock =  mAdapter.getDatas().get(mAdapter.getCount()-1);
               stock.setDate(data.getTime());
               stock.setClosePrice(data.getClosePrice());
               stock.setHighestPrice(data.getHighestPrice());
               stock.setLowestPrice(data.getLowestPrice());
               stock.setVolume(data.getVolume());
              List<Stock> list =  mAdapter.getDatas();
                       list.add(mAdapter.getCount()-1,stock);
              // stock.setHighestPrice(data.getHighestPrice());
                mAdapter.updateData(list);*/
            } /*else if(mAdapter.isGetMoreData()){
                switch (currenttype) {
                    case "1":
                    case "分时":
                        mPresenter.getKData(System.currentTimeMillis() - 2*24 * 60 * 60 * 1000L, "1", code, System.currentTimeMillis()- 24 * 60 * 60 * 1000L);
                        break;
                    case "5":
                        mPresenter.getKData(System.currentTimeMillis() - 2*24 * 60 * 60 * 1000L, "5", code, System.currentTimeMillis()- 24 * 60 * 60 * 1000L);
                        break;
                    case "15":
                        mPresenter.getKData(System.currentTimeMillis() - 2*5 * 24 * 60 * 60 * 1000L, "15", code, System.currentTimeMillis()- 5 * 24 * 60 * 60 * 1000L);
                        break;
                    case "30":
                        mPresenter.getKData(System.currentTimeMillis() - 5 * 24 * 60 * 60 * 1000L, "30", code, System.currentTimeMillis());
                        break;
                    case "1h":
                        mPresenter.getKData(System.currentTimeMillis() - 30 * 24 * 60 * 60 * 1000L, "60", code, System.currentTimeMillis());
                        break;
                    case "4h":
                        mPresenter.getKData(System.currentTimeMillis() - 60 * 24 * 60 * 60 * 1000L, "4h", code, System.currentTimeMillis());
                        break;
                    case "1d":
                        mPresenter.getKData(System.currentTimeMillis() - 300 * 24 * 60 * 60 * 1000L, "1d", code, System.currentTimeMillis());
                        break;
               ...
               /* switch (currenttype) {
                ...
                // mPresenter.getKData(System.currentTimeMillis()-300*24*60*60*1000L,currenttype,code,System.currentTimeMillis());
                ...
                DataUtil.calculate(mAdapter.getDatas());
                //mAdapter.changeLastItemClosePrice(Float.valueOf(String.valueOf(data.getClosePrice())));
               /* if (isFirst) {
                    isFirst = true;
    ...
    private boolean isSameTime(long compareTime) {
        ...
        long d = mAdapter.getDatas().get(mAdapter.getCount() - 1).getDate()/dividerTime;
        return compareTime / dividerTime == d;
    }
