...
import com.sskj.hangqing.presenter.NewHangActivityPresenter;
...
@Route(path = RConfig.NEWHANGACTIVITY)
public class NewHangActivity extends BaseActivity<NewHangActivityPresenter> {
    ...
    protected void initView() {
     ...
     //   fragments.add((Fragment) ARouter.getInstance().build(RConfig.MYSELECTFRAGMENT).navigation());
        fragments.add((Fragment) ARouter.getInstance().build(RConfig.STANDARDFRAGMENT).withBoolean("isLevel",isLevel).navigation());
    ...
    @Override
    public NewHangActivityPresenter getPresenter() {
        return new NewHangActivityPresenter();
    }
