...
public class BibiBuyAndSellFragment extends BaseFragment<BibiBuyAndSellFragmentPresenter> {
  ...
  /*  @BindView(R2.id.indicatorSeekBar)
        ...
        group.setOnCheckedChangeListener(new MyRadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(MyRadioGroup group, int checkedId) {
               ...
               if(isBuy){
                   ...
                   if(isMarketPrice){
                       etNum1.setText(new BigDecimal(usdtMoney).multiply(new BigDecimal(0.25)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                   }else {
                       etNum1.setText(new BigDecimal(usdtMoney).multiply(new BigDecimal(0.25)).divide(new BigDecimal(etPrice1.getText().toString()),BigDecimal.ROUND_DOWN).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                   }
                   ...
                   tvTradeMoney.setText(new BigDecimal(coinMoney).multiply(new BigDecimal(0.25)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                   etNum1.setText(new BigDecimal(coinMoney).multiply(new BigDecimal(0.25)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
               }
               }else if(button.getText().toString().equals("50%")){
                   if(isBuy){
                       if(isMarketPrice){
                           etNum1.setText(new BigDecimal(usdtMoney).multiply(new BigDecimal(0.5)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                       }else {
                           etNum1.setText(new BigDecimal(usdtMoney).multiply(new BigDecimal(0.5)).divide(new BigDecimal(etPrice1.getText().toString()),BigDecimal.ROUND_DOWN).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                       }
                       tvTradeMoney.setText(new BigDecimal(usdtMoney).multiply(new BigDecimal(0.5)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                   }else {
                       tvTradeMoney.setText(new BigDecimal(coinMoney).multiply(new BigDecimal(0.5)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                       etNum1.setText(new BigDecimal(coinMoney).multiply(new BigDecimal(0.5)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                   }
               }else if(button.getText().toString().equals("75%")){
                   if(isBuy){
                       if(isMarketPrice){
                           etNum1.setText(new BigDecimal(usdtMoney).multiply(new BigDecimal(0.75)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                       }else {
                           etNum1.setText(new BigDecimal(usdtMoney).multiply(new BigDecimal(0.75)).divide(new BigDecimal(etPrice1.getText().toString()),BigDecimal.ROUND_DOWN).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                       }
                       ...
                       tvTradeMoney.setText(new BigDecimal(coinMoney).multiply(new BigDecimal(0.75)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                       etNum1.setText(new BigDecimal(coinMoney).multiply(new BigDecimal(0.75)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                   }
               }else if(button.getText().toString().equals("100%")){
                   if(isBuy){
                       if(isMarketPrice){
                           etNum1.setText(new BigDecimal(usdtMoney).multiply(new BigDecimal(1)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                       }else {
                           etNum1.setText(new BigDecimal(usdtMoney).multiply(new BigDecimal(1)).divide(new BigDecimal(etPrice1.getText().toString()),BigDecimal.ROUND_DOWN).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                       }
                       tvTradeMoney.setText(new BigDecimal(usdtMoney).multiply(new BigDecimal(1)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                   }else {
                       tvTradeMoney.setText(new BigDecimal(coinMoney).multiply(new BigDecimal(1)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                       etNum1.setText(new BigDecimal(coinMoney).multiply(new BigDecimal(1)).setScale(4,BigDecimal.ROUND_DOWN).toPlainString());
                   }
               }
    ...
    public void initView() {
       ...
       /* switch (CoinUtil.getNumKeepNum(code)) {
            case 2:
                rate = "0.01";
                break;
            case 4:
    ...
    private void updateUI() {
       ...
       // indicatorSeekBar.setProgress(0);
        ...
        rgLevel.clearCheck();
        tvMoney.setText(String.format("%s%s", isBuy ? CoinUtil.keepCoinMoney(code, usdtMoney) : CoinUtil.keepCoinNum(code, coinMoney), isBuy ? moneyType : type));
        ...
        if (userData != null) {
            btBuy.setText(isBuy ? App.INSTANCE.getString(R.string.bibi_tipBibiUtil1) : App.INSTANCE.getString(R.string.bibi_recordActivity3));
            ClickUtil.click(btBuy, () -> {
                if (checkParam()) {
                    if(isBuy&&isMarketPrice){
                        mPresenter.submit(marketPrice,etNum1.getText().toString(),"BUY",code,"MARKET_PRICE");
                    }else if(isBuy&&!isMarketPrice){
                        mPresenter.submit(currentPrice,etNum1.getText().toString(),"BUY",code,"LIMIT_PRICE");
                    }else if(!isBuy&&isMarketPrice){
                        mPresenter.submit(marketPrice,etNum1.getText().toString(),"SELL",code,"MARKET_PRICE");
                    }else if(!isBuy&&!isMarketPrice){
                        mPresenter.submit(currentPrice,etNum1.getText().toString(),"SELL",code,"LIMIT_PRICE");
                    }
  ...
  /*  private void updateIndicator() {
        indicatorSeekBar.setProgress(0);
        indicatorSeekBar.setThumbDrawable(ContextCompat.getDrawable(App.INSTANCE, isBuy ? com.sskj.level.R.drawable.btn_background_round : com.sskj.level.R.drawable.btn_background_round2));
        indicatorSeekBar.setTickMarksDrawable(ContextCompat.getDrawable(App.INSTANCE, isBuy ? com.sskj.level.R.drawable.bibi_selector_marks_drawable_green : com.sskj.level.R.drawable.bibi_selector_marks_drawable_red));
        indicatorSeekBar.setProgressTrackColor(ContextCompat.getColor(App.INSTANCE, isBuy ? com.sskj.level.R.color.bibi_03c : com.sskj.level.R.color.bibi_ff5));
    }*/
    ...
    private void calculateMaxTrade() {
        if (TextUtils.isEmpty(currentPrice) || Double.valueOf(currentPrice) == 0) {
            ...
            if (isBuy) {
                if (isMarketPrice) {
                    maxTradeCoin =new BigDecimal(usdtMoney).stripTrailingZeros().toPlainString();
                } else {
                    maxTradeCoin = new BigDecimal(usdtMoney).stripTrailingZeros().toPlainString();
                }
            } else {
                maxTradeCoin = new BigDecimal(coinMoney).toString();
            }
        }
    ...
    private void changeShowData() {
        if (TextUtils.isEmpty(etPrice1.getText()) || TextUtils.isEmpty(etNum1.getText())) {
            if (isBuy) {
                ...
                if (isMarketPrice) {
                    tvAllMoneyTitle.setText(App.INSTANCE.getString(R.string.bibi_bibiBuyAndSellFragment15));
                } else {
                    tvAllMoneyTitle.setText(App.INSTANCE.getString(R.string.bibi_bibiBuyAndSellFragment14));
                }
            }
            ...
            if (!isMarketPrice) {
                ...
                if (isBuy) {
                   String num =  etNum1.getText().toString();
                    usdtFull = new BigDecimal(num)
                            .multiply(new BigDecimal(etPrice1.getText().toString())).toString();
                } else {
                    usdtFull = new BigDecimal(etNum1.getText().toString()).multiply(new BigDecimal(etPrice1.getText().toString())).toString();
                }
                String usdt;
                if (isBuy) {
                    //限价买入
                    ...
                    tvAllMoneyTitle.setText(App.INSTANCE.getString(R.string.bibi_bibiBuyAndSellFragment16));
                ...
                String usdtFull;
                if (isBuy) {
                    usdtFull = new BigDecimal("1").multiply(new BigDecimal(etNum1.getText().toString())).toString();
...
//                        isMarketPrice = true;
    ...
    private static final String TAG = "BibiBuyAndSellFragment";
...
//                            progress = progress > 1000 ? 1000 : progress;
     ...
     /*   RxTextView.textChanges(etPrice1)
                .subscribe(data -> {
                    String text = data.toString();
                    Log.e(TAG, "initInputChange2: "+text);
                    if (TextUtils.isEmpty(text)) {
...
//                            progress = progress > 1000 ? 1000 : progress;
    ...
    public void initData() {
      ...
      //  mPresenter.getCoinInfo(code, userData);
       ...
       // mPresenter.initSocket(code);
        ...
        LiveDataBus.get().with(RxBusCode.BIBI_CHANGE_COIN,BibiCoinType.class).observe(this, this::changeCoin);//修改币币交易中币种
        LiveDataBus.get().with(RxBusCode.BIBI_CHANGE_COIN2,BibiCoinType.class).observe(this, this::changeCoin1);//改变从marketactivity是买还是卖
    ...
    public void updateCoinData(CoinUpdateBean coinUpdateBean) {
        ...
        usdtMoney = coinUpdateBean.getRightMoney();
        coinMoney = coinUpdateBean.getLeftMoney();
...
String unit="$";
        ...
        if(isBuy){
            if(wsFiveBean.getAsk().getItems()!=null&&wsFiveBean.getAsk().getItems().size()>0){
                if(wsFiveBean.getAsk().getItems().size()>5){
                    WSFiveBean1.AskBean.ItemsBean bean =  wsFiveBean.getAsk().getItems().get(4);
                    marketPrice = bean.getPriceStr();
                }else {
                    WSFiveBean1.AskBean.ItemsBean bean =  wsFiveBean.getAsk().getItems().get(wsFiveBean.getAsk().getItems().size()-1);
                    marketPrice = bean.getPriceStr();
                }
            ...
            if(wsFiveBean.getBid().getItems()!=null&&wsFiveBean.getBid().getItems().size()>0){
                WSFiveBean1.BidBean.ItemsBeanX bean1= wsFiveBean.getBid().getItems().get(0);
                marketPrice = bean1.getPriceStr();
            }else {
                marketPrice="5000";
            }

        }
        if(!marketPrice.contains("5000")){
    ...
    public void getDealDetail(DealDetailBean bean) {
        fee = bean.getFee();
        minxianTradNum = new BigDecimal(bean.getMinTurnover()).setScale(4,BigDecimal.ROUND_DOWN).stripTrailingZeros().toPlainString();
        mincommonTypeTradNum = new BigDecimal(bean.getMinVolume()).setScale(4,BigDecimal.ROUND_DOWN).stripTrailingZeros().toPlainString();



       /* if(isBuy&&isMarketPrice){
            mPresenter.getwallet(minxianTradNum);
        }else {
    ...
    public void updateData(CoinDetailBean bean) {
        if(bean.getData()!=null&&bean.getData().size()>0){
            if(isBuy){
                usdtMoney = bean.getData().get(0).getBalance();
            }else {
                coinMoney = bean.getData().get(0).getBalance();
            }
    ...
    public void success(String message) {
     // String language =  LanguageUtil.getAppLocale(App.INSTANCE).toString();
        //ToastUtil.showShort(message);
        if(message.contains("成功")||message.contains("success")){
        ToastUtil.showShort(App.INSTANCE.getString(R.string.bibi_success));
        }
        refresh();
    }
